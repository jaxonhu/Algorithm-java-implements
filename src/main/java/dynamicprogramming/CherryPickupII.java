...
public class CherryPickupII {
    ...
    /**
     *
     * 动态规划： dp[i][p][q] 表示第i行，第一个机器人位于位置p, 第二个机器人位于位置q时 采摘到樱桃的最大数量
     *
     */
    public int cherryPickup(int[][] grid) {
        ...
        for(int i = 1 ; i < n ; i ++) {
            for(int p = 1 ; p <= i+1 && p <= m ; p ++) {
                for(int q = m ; q >= m-i  && q >= 1; q --) {
                    if(p >= q) continue;
                    int max = -1;
                    for(int k = p - 1 ; k <= p+1 ; k ++) {
                        for(int l = q - 1;  l <= q+1; l ++) {
                            if(k >= l) continue;
                            max = Math.max(max, dp[i-1][k][l]);
                        }
                    }
                    dp[i][p][q] = max + grid[i][p-1] + grid[i][q-1];
                }
            }
        }
        int res = -1;
